...
export default function EnrollFace() {
  ...
  async function captureOrSave() {
    ...
    try {
      ...
      else { await request("/attendance/me/face", { method: "POST", body: await selfieForm(uri) }); setDone(true); }
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Face could not be saved."); }
  ...
  return <CheckInSheet onDismiss={dismiss}>
    ...
    <ScrollView style={sheet.scroll} contentContainerStyle={sheet.content}>
      ...
      {error && <Notice actionLabel="Retake" onAction={() => { setUri(null); setReady(false); setError(null); }}>{error}</Notice>}
      {uri && !done && !pending && <Notice actionLabel="Retake" onAction={() => { setUri(null); setReady(false); }}>Blurry photo? Take another one.</Notice>}
    </ScrollView>
