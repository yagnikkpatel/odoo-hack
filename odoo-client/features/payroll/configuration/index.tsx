...
export default function PayrollConfiguration({ kind }: { kind:'rules'|'structures' }) {
 const dataState = usePayrollData();
  ...
  const data = useMemo<ConfigurationRow[]>(()=>{
    ...
    return isRules ? [...rules].sort((a,b)=>a.sequence-b.sequence).map(rule=>({...base,id:rule.id,name:rule.name,code:rule.code,category:RULE_CATEGORIES[rule.category],sequence:rule.sequence,method:COMPUTATION_METHODS[rule.method],computation:calculation(rule),status:rule.active?'Active':'Inactive'})) : structures.map(structure=>({...base,id:structure.id,name:structure.name,rules:structure.ruleIds.length,employees:structure.employeeCount ?? 0,status:structure.active?'Active':'Inactive'}))
  ...
  const columns = useMemo<ColumnDef<ConfigurationRow>[]>(()=>[
    ...
    {id:'actions',size:48,enableSorting:false,enableHiding:false,cell:({row})=>(canEdit || canDelete)?<RowActionShell label={singular+' actions'} onEdit={canEdit?()=>setEditor(row.original.id):undefined} onDelete={canDelete?async ()=>{
      const store=usePayrollStore.getState()
      const result=await (isRules?store.removeRule(row.original.id):store.removeStructure(row.original.id))
  ...
  if (!canRead) return <div className={PAGE_BODY}><PayrollDataStatus state={dataState} /><p className="text-muted-foreground text-sm">Your role does not have access to payroll configuration.</p></div>
  return <div className="flex min-h-full flex-col">
    ...
    <div className={PAGE_BODY}><PayrollDataStatus state={dataState}/><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-muted-foreground text-xs">{isRules?'Rules execute in sequence to calculate earnings, deductions and net salary.':'Choose a structure when creating a payrun to apply its salary rules.'}</p></div><RecordsTable table={table} columnIds={columnIds} loading={dataState.isLoading} label={title.toLowerCase()} noun={isRules?'salary rule':'salary structure'} onOpen={row=>setSelectedId(row.id)}/></div>
