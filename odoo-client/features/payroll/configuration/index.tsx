...
export default function PayrollConfiguration({ kind }: { kind:'rules'|'structures' }) {
  ...
  return <div className="flex min-h-full flex-col">
    ...
    <div className={PAGE_BODY}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-muted-foreground text-xs">{isRules?'Rules execute in sequence to calculate earnings, deductions and net salary.':'Choose a structure when creating a payrun to apply its salary rules.'}</p></div><RecordsTable table={table} columnIds={columnIds} loading={false} label={title.toLowerCase()} onOpen={row=>setSelectedId(row.id)}/></div>
