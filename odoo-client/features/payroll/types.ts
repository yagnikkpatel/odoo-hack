...
export type Payslip = { id: string; payrunId: string; employeeId: string; employeeName: string; employeeEmail: string; department: string; employmentType: string; structureId: string; structureName: string; startDate: string; endDate: string; status: PayrollStatus; currency: string; workedDays: number; workedHours: number; expectedDays: number; expectedHours: number; basic: number; allowances: number; deductions: number; contributions: number; gross: number; net: number; lines: PayslipLine[]; warnings: PayrollWarning[]; contractSnapshot?: import('./contract-input').PayrollContractInput; bankAccount?: string }
