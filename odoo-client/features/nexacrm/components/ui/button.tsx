...
function Button({
  ...
  // The template renders navigation links through Button. Preserve anchor semantics.
  const isLink = isValidElement(render) && (render.type === Link || render.type === 'a')

  return <ButtonPrimitive data-slot='button' className={cn(buttonVariants({ variant, size, className }))}
    render={render} nativeButton={nativeButton ?? !isLink} role={isLink ? 'link' : undefined} {...props} />
}
