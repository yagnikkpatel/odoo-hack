...
const PersonDetailTabs = ({
  ...
  const feedCount = useRecordFeed('person', entityId).length
  const taskCount = useEntityTasks('person', entityId).length
  const noteCount = useEntityNotes('person', entityId).length
  const fileCount = useEntityAttachments('person', entityId).length
  const emailCount = useEntityEmails('person', entityId).length
  const eventCount = useEntityCalendarEvents('person', entityId).length
  ...
  const tabs = [
    {
      ...
      icon: ClockIcon,
      count: feedCount,
      content: <TimelinePanel entityType='person' entityId={entityId} onOpenNote={openNote} />
    },
    {
      value: 'tasks',
      label: 'Tasks',
      icon: ListTodoIcon,
      count: taskCount,
      content: <TasksPanel entityType='person' entityId={entityId} onOpenTask={openTask} />
    },
    {
      value: 'notes',
      label: 'Notes',
      icon: StickyNoteIcon,
      count: noteCount,
      content: <NotesPanel entityType='person' entityId={entityId} onOpenNote={openNote} />
    },
    {
      value: 'files',
      label: 'Files',
      icon: PaperclipIcon,
      count: fileCount,
      content: <FilesPanel entityType='person' entityId={entityId} />
    },
    {
      value: 'emails',
      label: 'Emails',
      icon: MailIcon,
      count: emailCount,
      content: <EmailsPanel entityType='person' entityId={entityId} onOpenEmail={openEmail} onCompose={composeEmail} />
    },
    {
      value: 'calendar',
      label: 'Calendar',
      icon: CalendarIcon,
      count: eventCount,
      content: <CalendarPanel entityType='person' entityId={entityId} />
    }
  ]
