...
// Local Imports
import { HIDDEN_BY_DEFAULT_COLUMNS, INITIAL_COLUMN_ORDER, columns } from './columns'

const DEFAULT_PAGE_SIZE = 15

export const useOpportunitiesTable = ({ onEditOpportunity }: { onEditOpportunity: (o: Opportunity) => void }) => {
  const opportunities = useOpportunitiesStore(state => state.opportunities)

  const [sorting, setSorting] = useState<SortingState>([])
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])

  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>(HIDDEN_BY_DEFAULT_COLUMNS)
  ...
  const meta = useMemo(() => ({ onEditRow: onEditOpportunity }), [onEditOpportunity])
