...
const SALARY_RULES: RuleSpec[] = [
  ...
  { name: "Executive Fixed Allowance", code: "FIX_EXEC", category: "allowance", sequence: 51, method: "formula", formula: "WAGE - BASIC - HRA - STD - LTA - NET_ALLOW" },
  { name: "Executive Gross Salary", code: "GROSS_EXEC", category: "gross", sequence: 62, method: "formula", formula: "BASIC + HRA + STD + LTA + NET_ALLOW + BONUS + FIX_EXEC" },
  { name: "Executive Loss of Pay", code: "LOP_EXEC", category: "deduction", sequence: 71, method: "formula", formula: "GROSS_EXEC / PERIOD_DAYS * UNPAID_DAYS" },
  { name: "Executive Income Tax", code: "TDS_EXEC", category: "deduction", sequence: 103, method: "percentage", percentage: 8, base: "GROSS_EXEC" },
  { name: "Executive Net Salary", code: "NET_EXEC", category: "net", sequence: 123, method: "formula", formula: "GROSS_EXEC - LOP_EXEC - PF - PT - TDS_EXEC" },
...
const SALARY_STRUCTURES: StructureSpec[] = [
  ...
  },
  {
    ...
    description: "Leadership salary with no overtime component.",
    codes: ["BASIC", "HRA", "STD", "LTA", "NET_ALLOW", "BONUS", "FIX_EXEC", "GROSS_EXEC", "LOP_EXEC", "PF", "PT", "TDS_EXEC", "PF_EMPR", "NET_EXEC"],
  },
];
...
async function insertContracts(client: PoolClient, people: SeedUser[]): Promise<number> {
  ...
  for (const person of people) {
    ...
    // Keep historical contracts entirely before the demo payroll window.
    // Never backdate a running contract over an earlier expired contract.
    const earliestPayroll = new Date(TODAY.getFullYear(), TODAY.getMonth() - PAYROLL_MONTHS, 1);
    const start = joined > earliestPayroll ? joined : earliestPayroll;
    const historyDays = Math.round((start.getTime() - joined.getTime()) / 86_400_000);
    const priorContracts = historyDays > 600 ? 2 : historyDays > 0 ? 1 : 0;
    let cursor = joined;

    for (let i = 0; i < priorContracts; i++) {
      const nextStart = i === priorContracts - 1
        ? start
        : addDays(joined, Math.floor(historyDays * (i + 1) / priorContracts));
      ...
      await seedContract(client, {
        ...
        endDate: isoDate(end),
        wage: Math.round(person.wage * (0.72 + i * 0.12)),
        status: "expired",
        createdAt: cursor,
        updatedAt: end,
      });
    ...
    if (person.status === "inactive") {
      const leftOn = addDays(TODAY, -randomInt(5, Math.min(90, tenure - 1)));
  ...
  for (const person of people) {
    ...
    for (let daysAgo = ATTENDANCE_DAYS; daysAgo >= 0; daysAgo--) {
      ...
      if (roll < (daysAgo === 0 ? absenteeism * 0.35 : absenteeism)) {
        ...
        // Demo attendance is settled, including today's sample rows, so
        // computing any seeded payrun never encounters a missing check-out.
...
/** Reset only this application's caches/sessions and its outbound payroll jobs. */
async function clearSeedRuntimeState(): Promise<void> {
  const queue = new Queue("payslip-email", { connection: bullmqConnection });
  try {
    if (await queue.getActiveCount()) {
      throw new Error("Stop the payslip email worker before resetting demo data.");
    }
    // Pause before deletion so a connected worker cannot claim waiting jobs.
    await queue.pause();
  ...
  const patterns = [
    "auth:refresh:*", "password-reset:*", "users:*", "user:*",
...
async function verifySeed(): Promise<void> {
  const { rows: [result] } = await pool.query(`
    SELECT
      ...
      (SELECT count(*)::int FROM payslip_deliveries) AS deliveries,
  ...
  if (Object.values(result).some((count) => count !== 0)) {
    throw new Error(`Demo data verification failed: ${JSON.stringify(result)}`);
  }
  logger.info(result, "demo data checks passed");
}
...
async function seed(): Promise<void> {
  ...
  for (const [name, id] of structures!) {
    const error = validateRuleSequence((await findRulesForStructure(id)).filter((rule) => rule.active));
    if (error) throw new Error(`Invalid salary structure ${name}: ${error}`);
  }
